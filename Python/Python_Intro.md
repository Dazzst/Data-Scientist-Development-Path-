...
• 无视觉参考的工作方式：不同于Excel 可以随时看到所有数据，在Python 中处理几百万行数据时，我们不会一直盯着数据看。我们只需在脑海中对数据的形态（Shape）和统计属性保持认知，这种方式极大提升了处理速度。

• 单元格 (Cells)：笔记本由单元格组成，主要分为代码 (Code) 和 Markdown (格式化文本) 两种类型。这让你可以将分析代码和精美的文本报告结合在一起导出为 PDF 或 HTML。
